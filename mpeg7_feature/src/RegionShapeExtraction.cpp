...
inline void RegionShapeExtractionTool::GenerateBasisLUT()
{
	...
	{
		radius = HYPOT(static_cast<long double>(x-maxradius), static_cast<long double>(y-maxradius));
		if(radius < maxradius)
		{
			angle = atan2(static_cast<long double>(y-maxradius), static_cast<long double>(x-maxradius));
